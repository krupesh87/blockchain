...
def transaction():
    ...
    print(balance,session.get('username'))
...
def transaction1():
    ...
    if request.method == 'POST':
        try:
            #attempt to execute the transaction
            send_money(session.get('username'),"Krupesh Patel", form.amount.data)
            flash("Money Sent!", "success")
        except Exception as e:
            flash(str(e), 'danger')

        return redirect(url_for('transaction1'))

    return render_template('product.html', balance=balance, form=form, page='transaction1')
#Index page
@app.route("/")
@app.route("/index")
